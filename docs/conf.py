...
def create_antsibull_docs(files, plugin_type=None):
    ...
    for f in files:
        ...
        if plugin_type is not None:
            args_string = f"junk plugin --dest-dir {file_path} --plugin-type {plugin_type} infrahub.infrahub.{file_name}"
        else:
            args_string = f"junk plugin --dest-dir {file_path} --plugin-type module infrahub.infrahub.{file_name}"
