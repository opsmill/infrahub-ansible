...
Get inventory hosts from Infrahub. When strict is false, a compose, groups or keyed_groups expression that fails to resolve emits a warning naming the host and the expression instead of failing silently. 
