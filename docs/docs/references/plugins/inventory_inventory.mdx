...
Get inventory hosts from Infrahub. When strict is false, a compose, groups or keyed_groups expression that fails to resolve emits one warning per expression naming the affected hosts, instead of failing silently. 
