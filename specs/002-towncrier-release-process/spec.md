...
- **The release pull request is enforced; approval on it is not.** `release-publish.yml` tags and publishes only when the pushed commit is the merge of a `release/*` pull request — it asks the API which pull requests the commit belongs to, so a direct push to `stable`, a hand-edited version or the version-sync commit is a clean no-op rather than a release. That covers the "reviewable pull request" half of FR-008 in the workflow itself. The workflow cannot tell an approved merge from an unapproved one, so the "requires approval" half still depends on branch protection for `stable` requiring an approving review — a repository setting, not a file in this change. At specification time `stable` required 0 approving reviews and 0 status checks, so FR-008 is only fully satisfied once that is configured; until then the workflow guard is the enforced control and branch protection is the defence in depth on top of it.
